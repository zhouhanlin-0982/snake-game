class SnakeView {
    constructor(model) {
      ...
      this.replayHud = document.getElementById("replayHud");
      this.replayStatus = document.getElementById("replayStatus");
      this.replayProgressBar = document.getElementById("replayProgressBar");
      this.replaySpeedLabel = document.getElementById("replaySpeedLabel");
      this.btnReplayPause = document.getElementById("btnReplayPause");
      this.btnReplayStop = document.getElementById("btnReplayStop");
      this.btnReplaySlow = document.getElementById("btnReplaySlow");
      this.btnReplayFast = document.getElementById("btnReplayFast");
    ...
    showGameOverScreen(reason, score, hasReplay) {
      this.pageDifficulty.hidden = true;
      this.pageStandard.hidden = true;
      ...
      // 控制回放按钮显示
      if (this.btnReplay) {
        this.btnReplay.hidden = !hasReplay;
      }
    }
    ...
    // ========== 回放 HUD 方法 ==========
    showReplayHud(totalFrames) {
      if (this.replayHud) {
        this.replayHud.hidden = false;
        this.replayStatus.textContent = `回放 1 / ${totalFrames}`;
      }
      this.hideOverlay(); // 确保画布可见
    }
  
    hideReplayHud() {
      if (this.replayHud) this.replayHud.hidden = true;
    }
  
    updateReplayProgress(current, total) {
      if (this.replayProgressBar) {
        const pct = (current / total) * 100;
        this.replayProgressBar.style.width = pct + "%";
      }
      if (this.replayStatus) {
        this.replayStatus.textContent = `回放 ${current} / ${total}`;
      }
    }
  
    updateReplaySpeedLabel(speed) {
      if (this.replaySpeedLabel) {
        this.replaySpeedLabel.textContent = speed.toFixed(1) + "x";
      }
    }
  
    syncReplayPauseButton(isPaused) {
      if (this.btnReplayPause) {
        this.btnReplayPause.textContent = isPaused ? "继续" : "暂停";
      }
    }
